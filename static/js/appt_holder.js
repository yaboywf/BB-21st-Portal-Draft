const socket = io.connect('/appt_holder_room', { query: "username=" + document.getElementById("current_user").getAttribute("username") });

async function get_user_data() {
    await fetch("/get_all_officers", {
        method: "GET",
        headers: {
    ...
    .then(data => {
        ...
        data.forEach(user => {
            ...
            user_option.textContent = `${user.rank === "NIL" ? user.honorifics : user.rank} ${user.user_name}`;
            user_option.value = user.id;
            document.getElementById("captain").appendChild(user_option);
        })
    })
    .catch(error => {
        add_error(error);
...
socket.on('connect', function() {
    console.log("Connected to appointment holder room");
